...
| `RANGE=THISANDFUTURE` splitting in `ical-itip`, and `ValueKind` against `ical_core::ValueType` | `docs/design/ical-itip-api.md:530`, `docs/design/ical-recur-api.md:652` | Partially closed in the superseding single-crate facade: an organizer `REQUEST` is authorized against the master, checked against its recurrence set and zone policy, materialized transactionally as a detached anchor, updated in place on a later revision, and consumed by CalDAV recurrence evaluation. Other method-specific range behavior remains. The `ValueKind` deletion is conditional on a trigger — `ical-core` narrowing its own value type — that has not fired. | Debt (partial) |
